...
logger = get_logger('futuresreport.report_agent')
...
class ReportConsoleLogger:
    ...
    def _setup_file_handler(self):
        ...
        loggers_to_attach = [
            'futuresreport.report_agent',
            'futuresreport.zep_tools',
            'futuresreport.llm_client',
        ]
...
【未来学核心框架：未来锥（Cone of Plausibility）】
从单一模拟基础，你必须识别并分析三种未来：
- 可能的未来（Possible）：逻辑上可以发生，但概率相对较低
- 似乎的未来（Plausible）：根据现有驱力与趋势，可预期的发展方向
- 渴望的未来（Preferred）：从特定利益相关者角度，希望实现的未来

【STEEP分析维度】
每份报告必须至少检视以下部分维度的预测发现：
社会（Social）/ 科技（Technological）/ 环境（Environmental）/ 经济（Economic）/ 政治（Political）

【报告必须包含的核心要素】
1. 驱力识别（Driving Forces）：推动或阻碍未来走向的核心趋势与力量
2. 浮现议题（Emerging Issues）：模拟中出现的边缘行为与异常信号，可能预示重大转折
3. 情节范畴（Scenario Spectrum）：至少涵盖两条不同发展路径（多种未来／alternative futures）
4. 时间分层预测：区分近期（0-1年）/ 中期（1-5年）/ 远期（5-10年）的影响

【因果分层分析法（CLA）深度分析框架】
适用于驱力分析章节，从四个层次展开分析：
- 表象层（Litany）：可见事件与统计数字
- 系统层（Systemic）：驱动行为背后的社会／政治／技术系统
- 世界观（Worldview）：驱动行为的信念与价值体系
- 迷思／隐喻层（Myth/Metaphor）：深层文化结构与象征意义

【章节设计指导原则】
- 最少2个章节，最多5个章节
- 章节结构应体现多种未来的完整分析框架，而非单一时间线陈述
- 推荐章节架构（可根据模拟结果灵活调整）：
  * 驱力与浮现议题识别（含CLA分析）
  * 如常情节（Business as Usual）：基线未来预测
  * 多种未来：情节范畴（最佳想象／最糟想象）
  * 驟变情节与不确定性分析
  * 渴望的未来与近中远期行动指南
...
{
    "title": "报告标题",
    "summary": "报告摘要（融合未来锥视角，概括核心预测发现与主要情节走向）",
    "sections": [
        {
            "title": "章节标题",
            "description": "章节内容描述（注明该章节将应用的未来学分析维度）"
        }
    ]
}

注意：sections数组最少2个，最多5个元素！每章节标题应清晰体现未来学方法论视角。"""
...
【未来学分析指令】
以「上帝视角」审视这个未来预演，在规划报告大纲时，请同步识别：

1. 关键驱力（Key Driving Forces）：
   影响模拟走向最重要的2-3个趋势或力量（社会、科技、经济、政治、环境面向）

2. 双变数情节分析（Double Variable Scenario Matrix）：
   选取两个最不确定、最重要的驱力，构成四类情节：
   - 最佳想象（Best Case）：驱力皆向好的方向演化
   - 最糟想象（Worst Case）：驱力朝负面方向扩大
   - 如常情节（Business as Usual）：即当前模拟的基线结果
   - 驟变情节（Outlier）：低概率但高冲击的浮现议题导致的突发转折

3. 浮现议题清单（Emerging Issues Inventory）：
   模拟中出现的边缘行为或异常信号（少数Agent的非主流反应），可能预示未来重大转折点

4. 时间分层预测（Temporal Layering）：
   区分近期（0-1年）/ 中期（1-5年）/ 远期（5-10年）的影响与综合预测发现

报告章节设计必须体现多种未来的完整分析框架，而非单一时间线的线性陈述。

【再次提醒】报告章节数量：最少2个，最多5个，内容精炼聚焦于核心预测发现。"""
...
- insight_forge: 深度洞察分析，多维度检索事实与关系；识别驱力与浮现议题的首选工具
- panorama_search: 广角全景搜索，了解事件全貌与演变过程；适合绘制时间分层预测（近/中/远期）脉络
- quick_search: 快速验证特定信息点；适合确认驟变情节的触发条件与关键证据
- interview_agents: 采访不同角色Agent，获取第一人称观点；挖掘世界观层与渴望的未来视角
...
在使用工具获取数据后，依据章节标题选用以下分析视角：

**驱力／浮现议题类章节** — 应用因果分层分析法（CLA）四层次：
- 表象层（Litany）：模拟中可见的事件、Agent 行为与统计数字
- 系统层（Systemic）：驱动行为背后的社会／政治／技术结构
- 世界观（Worldview）：Agent 行为背后的信念与价值体系
- 迷思／隐喻层（Myth/Metaphor）：深层文化结构与象征意义

**情节／多种未来类章节** — 区分三类未来并标记驟变：
- 可能的未来（Possible）：逻辑可发生，概率较低
- 似乎的未来（Plausible）：沿现有驱力发展的预期走向
- 渴望的未来（Preferred）：利益相关者希望实现的方向
- 驟变情节（Outlier）：低概率但高冲击的突发转折，须明确标记

**所有章节通用指引**：
- 主动识别浮现议题：少数 Agent 的边缘行为，可能预示重大转折
- 从 STEEP 维度（社会／科技／环境／经济／政治）系统检视模拟发现
- 区分近期（0-1年）／中期（1-5年）／远期（5-10年）的时间层次
...
class ReportAgent:
    ...
    def plan_outline(
        ...
        try:
            ...
            # 返回默认大纲（fallback）— 采用未来学标准结构
            return ReportOutline(
                title="未来预测报告",
                summary="基于模拟数据的多种未来情节分析",
                sections=[
                    ReportSection(title="驱力识别与浮现议题"),
                    ReportSection(title="如常情节：基线未来预测"),
                    ReportSection(title="多种未来：情节范畴"),
                    ReportSection(title="驟变情节与不确定性分析"),
                ]
            )
    ...
    def generate_report(
        ...
        Args:
            ...
            resume: 是否启用断点续跑。为 True 时，若同一 report_id 目录下已存在
                outline.json 和部分已完成的章节文件，则跳过大纲规划，从第一个
                未完成章节继续生成，避免因限流重试而重复消耗 LLM 配额。
        ...
        try:
            ...
            # 断点续跑检测：若已存在大纲和部分已完成章节，则跳过规划阶段，
            # 从第一个未完成章节继续生成（避免限流重试时重复消耗配额）
            outline = None
            resume_from_index = 0  # 0-based，从该索引开始生成
            generated_sections = []  # 保存内容用于上下文

            if resume:
                existing_outline = ReportManager.load_outline(report_id)
                if existing_outline and existing_outline.sections:
                    existing_sections = {
                        s["section_index"]: s
                        for s in ReportManager.get_generated_sections(report_id)
                    }
                    total_existing = len(existing_outline.sections)
                    first_missing = next(
                        (idx for idx in range(1, total_existing + 1) if idx not in existing_sections),
                        total_existing + 1
                    )
                    if first_missing > 1:
                        outline = existing_outline
                        resume_from_index = first_missing - 1
                        for idx in range(1, first_missing):
                            completed_section_titles.append(outline.sections[idx - 1].title)
                            generated_sections.append(existing_sections[idx]["content"].strip())
                        logger.info(
                            f"检测到断点，report_id={report_id} 从第 {first_missing} 章节继续生成"
                            f"（已完成 {first_missing - 1}/{total_existing}）"
                        )
            ...
            if outline is not None:
                ...
                ReportManager.update_progress(
                    ...
                    f"检测到断点，从第 {resume_from_index + 1} 章节继续生成",
...
class ReportManager:
    ...
    def load_outline(cls, report_id: str) -> Optional[ReportOutline]:
        ...
        Returns:
            ReportOutline，若 outline.json 不存在则返回 None
        """
