    ...
    请求（JSON）：
        {
            ...
            "graph_id": "fr_xxxx",    // 可选，如不提供则从project获取
...
def prepare_simulation():
    ...
    try:
        ...
        state.agent_count = state.entities_count
        ...
        def run_prepare():
            ...
            try:
                ...
                # 更新 phase
                try:
                    s2 = manager.get_simulation(simulation_id)
                    if s2:
                        s2.phase = "ready"
                        s2.progress_percent = 100
                        manager._save_simulation_state(s2)
                except Exception:
                    pass
...
def start_simulation():
    ...
    try:
        ...
        # 启动模拟 (支持 resume / start_round 实现 resumability)
        run_state = SimulationRunner.start_simulation(
            ...
            graph_id=graph_id,
            llm_config=llm_config,
            start_round=int(start_round) if start_round else 0,
            resume=bool(resume)
        )
        ...
        if run_state and run_state.current_round:
            state.current_round = run_state.current_round
            state.phase = f"simulating_round_{run_state.current_round}"
        state.progress_percent = run_state.progress_percent if hasattr(run_state, 'progress_percent') else 0
