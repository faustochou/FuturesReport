...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    if llm_provider == "anthropic":
        ...
        model_platform = getattr(ModelPlatformType, "ANTHROPIC", None)
        if model_platform is None:
            raise ValueError("Claude provider requires a CAMEL version with ModelPlatformType.ANTHROPIC support.")
    elif llm_api_key:
        os.environ["OPENAI_API_KEY"] = llm_api_key
    
    if llm_provider == "anthropic" and not os.environ.get("ANTHROPIC_API_KEY"):
        raise ValueError("Missing Anthropic API key. Please set your Claude API key in account settings.")
    if llm_provider != "anthropic" and not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("缺少 API Key 配置，请在项目根目录 .env 文件中设置 LLM_API_KEY")
    
    if llm_provider == "anthropic" and llm_base_url:
...
async def run_twitter_simulation(
    ...
    result.env = oasis.make(
        ...
        semaphore=semaphore,  # 动态来自 env / Config (低资源)
    )
    ...
    # resume 支持：从 start_round 开始（保持绝对 round 编号）
        ...
        if ckpt_interval > 0 and (round_num + 1) % ckpt_interval == 0:
            try:
                ckpt = {
                    ...
                    "platform": "twitter" if "twitter" in str(log_info) else "reddit",
                }
                ckpt_path = os.path.join(simulation_dir, f"checkpoint_{'twitter' if 'twitter' in str(log_info) else 'reddit'}.json")
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='从指定轮次恢复/开始模拟（用于 resumability）'
    )
    parser.add_argument(
        '--resume',
        action='store_true',
    ...
    start_round = args.start_round or 0
    ...
    if args.twitter_only:
        twitter_result = await run_twitter_simulation(config, simulation_dir, twitter_logger, log_manager, args.max_rounds, start_round)
    elif args.reddit_only:
        reddit_result = await run_reddit_simulation(config, simulation_dir, reddit_logger, log_manager, args.max_rounds, start_round)
    else:
        # 并行运行（每个平台使用独立的日志记录器）
        results = await asyncio.gather(
            run_twitter_simulation(config, simulation_dir, twitter_logger, log_manager, args.max_rounds, start_round),
            run_reddit_simulation(config, simulation_dir, reddit_logger, log_manager, args.max_rounds, start_round),
        )
