...
class SimulationRunState:
    ...
    # 资源与 checkpoint（低资源保护 + resumability）
...
class SimulationRunner:
    ...
    def _load_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        ...
        try:
            ...
            state = SimulationRunState(
                ...
                last_checkpoint_round=data.get("last_checkpoint_round", 0),
    ...
    @staticmethod
    def _apply_llm_environment(env: Dict[str, str], llm_config: Dict[str, str]) -> None:
    ...
    def start_simulation(
        ...
        start_round: int = 0,  # 用于 resume，从此轮开始（0=全新）
        resume: bool = False
    ) -> SimulationRunState:
        """
        启动模拟
        ...
        # 全局上限保护 (env 可覆盖)
        max_rounds_cap = max_rounds if (max_rounds is not None and max_rounds > 0) else Config.SIM_MAX_ROUNDS
        if max_rounds_cap and max_rounds_cap > 0:
            original = total_rounds
            total_rounds = min(total_rounds, max_rounds_cap)
            if total_rounds < original:
                logger.info(f"轮数已按上限截断: {original} -> {total_rounds}")
        
        # 如果指定 resume，使用现有 current_round 或传入 start_round
        effective_start = start_round
        existing_state = cls.get_run_state(simulation_id)
        if resume and existing_state:
            effective_start = max(effective_start, existing_state.current_round or 0, existing_state.last_checkpoint_round or 0)
            logger.info(f"Resume from round {effective_start}")
        
        # 记录到 config 供子进程使用（limits）
        config['limits'] = {
            'max_agents': Config.SIM_MAX_AGENTS,
            'llm_concurrency': Config.LLM_CONCURRENCY_LIMIT,
            'checkpoint_interval': Config.SIM_CHECKPOINT_INTERVAL,
        }
        ...
        state = SimulationRunState(
            ...
            current_round=effective_start,
            last_checkpoint_round=effective_start,
        )
        ...
        try:
            ...
            # 传递限制和 resume 参数 (仅 parallel 脚本当前支持 resume/ start-round；单平台脚本将忽略 start)
            if max_rounds_cap and max_rounds_cap > 0:
                cmd.extend(["--max-rounds", str(max_rounds_cap)])
            if platform == 'parallel':
                if effective_start > 0:
                    cmd.extend(["--start-round", str(effective_start)])
                if resume:
                    cmd.extend(["--resume"])
            ...
            # 低资源并发与限制（子进程读取用于 semaphore / caps）
            env['LLM_CONCURRENCY_LIMIT'] = str(Config.LLM_CONCURRENCY_LIMIT)
            env['SIM_MAX_AGENTS'] = str(Config.SIM_MAX_AGENTS)
            env['SIM_CHECKPOINT_INTERVAL'] = str(Config.SIM_CHECKPOINT_INTERVAL)
            if llm_config:
                cls._apply_llm_environment(env, llm_config)
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        try:
            while process.poll() is None:  # 进程仍在运行
                ...
                # 资源采样 + checkpoint（低资源保护）
                if psutil and process.pid:
                    try:
                        p = psutil.Process(process.pid)
                        mem = p.memory_info().rss / (1024 * 1024)
                        state.peak_memory_mb = max(state.peak_memory_mb or 0, mem)
                        # 每隔几秒记日志（避免刷屏）
                        if int(time.time()) % 30 == 0:
                            logger.info(f"[Resource] sim={simulation_id} child_mem_mb={mem:.1f} peak={state.peak_memory_mb:.1f}")
                    except Exception:
                        pass
                
                # 更新状态 + 周期性 checkpoint
                if state.current_round and state.current_round > (state.last_checkpoint_round or 0):
