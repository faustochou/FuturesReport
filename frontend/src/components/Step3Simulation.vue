<template>
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      ...
      <!-- 低资源/网络容错控制条 + 进度提示 -->
      <div class="sim-control-bar" v-if="phase === 1 || backendUnreachable || resourceWarning">
        <div v-if="backendUnreachable" class="unreachable-banner">
          ⚠ 后端不可达 (重试 {{ unreachableRetries }}). 模拟可能仍在后台运行。 
          <button @click="fetchRunStatus" class="mini-btn">立即重试</button>
          <button @click="doResumeSimulation" class="mini-btn" :disabled="isStarting">尝试恢复</button>
        </div>
        <div v-if="resourceWarning" class="resource-banner">{{ resourceWarning }}</div>
        
        <div class="control-buttons" v-if="phase === 1">
          <button @click="handleStopSimulation" class="mini-btn danger" :disabled="isStopping">停止模拟</button>
          <button @click="doResumeSimulation" class="mini-btn" :disabled="isStarting || !runStatus.current_round">从检查点恢复</button>
          <span class="progress-mini" v-if="runStatus.total_rounds">
            {{ runStatus.current_round || 0 }} / {{ runStatus.total_rounds }} ({{ runStatus.progress_percent || 0 }}%)
            <span v-if="runStatus.peak_memory_mb"> | mem: {{ Math.round(runStatus.peak_memory_mb) }}MB</span>
          </span>
        </div>
      </div>
    </div>
...
import { authState, hasLlmConfig, loadCurrentUser } from '../store/auth'
...
const unreachableRetries = ref(0)
...
const doStartSimulation = async () => {
  ...
  if (!authState.user) {
    startError.value = t('auth.loginRequiredEngine')
  ...
  if (!hasLlmConfig()) {
    startError.value = t('auth.llmRequiredEngine')
    addLog(startError.value)
    emit('update-status', 'error')
    return
  }
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // 强制重新开始 (可通过 resume 覆盖)
    ...
    if (res.success && res.data) {
      ...
      if (res.data.peak_memory_mb) addLog(`  └─ Peak mem: ${res.data.peak_memory_mb}MB`)
      
      phase.value = 1
      runStatus.value = res.data
      backendUnreachable.value = false
      unreachableRetries.value = 0
    ...
    if (err.isNetworkError || err.message === 'BACKEND_UNREACHABLE') {
      backendUnreachable.value = true
      addLog('BACKEND_UNREACHABLE: 后端暂时不可达，模拟进程可能仍在后台运行。正在重试...')
      // 不要立即 error，继续让用户有机会 resume 或等
    } else {
...
const doResumeSimulation = async () => {
  ...
  isStarting.value = true
  startError.value = null
  addLog(t('log.resumingSim') || 'Resuming simulation from last checkpoint...')
  try {
    const last = runStatus.value?.current_round || runStatus.value?.last_checkpoint_round || 0
    const params = {
      simulation_id: props.simulationId,
      platform: 'parallel',
      start_round: last,
    ...
    if (res.success) {
      addLog(t('log.engineStarted') || 'Resumed')
      phase.value = 1
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        ...
        const errMsg = data.error || t('common.unknownError') || 'unknown error'
        addLog(t('log.simFailed', { error: errMsg }))
    ...
    const isNetErr = !err.response || err.message === 'BACKEND_UNREACHABLE' || err.isNetworkError
    if (isNetErr) {
      backendUnreachable.value = true
      unreachableRetries.value += 1
      console.warn('Run status poll unreachable (retrying):', err.message)
      if (unreachableRetries.value % 5 === 0) {
        // 偶尔探测 health
        try { await getHealth() } catch (_) {}
      }
    } else {
...
/* 低资源保护 UI 元素 */
.sim-control-bar { margin: 8px 0; padding: 6px 10px; border-radius: 4px; background: rgba(255,193,7,0.1); font-size: 12px; }
.unreachable-banner, .resource-banner { color: #b45309; margin-bottom: 4px; }
