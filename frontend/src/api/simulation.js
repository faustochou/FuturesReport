...
/**
 * 启动模拟
 * @param {Object} data - { simulation_id, platform?, max_rounds?, enable_graph_memory_update?, start_round?, resume? }
 */
...
/**
 * 恢复/续跑模拟（resume from checkpoint）
 */
export const resumeSimulation = (data) => {
  const payload = { ...data, resume: true }
  return requestWithRetry(() => service.post('/api/simulation/start', payload), 3, 1000)
}

/**
 * 健康检查（用于 unreachable 恢复探测）
 */
export const getHealth = () => service.get('/health')
